  ...
  Future<void> _load() async {
    ...
    try {
      final detail = await ApiClient.instance
          .getTickerDetail(widget.providerTicker ?? widget.ticker);
          ...
          children: [
            ...
            Expanded(
              child: Text(
                ...
                '${dataSourceLabel(detail.priceSource)}',
    ...
    return Card(
      child: ExpansionTile(
        ...
        children: [
          ...
          _dataRow('Pattern storici comparabili', detail.recoveryPotential),
          ...
          _dataRow(
            detail.peBasis.isNotEmpty ? 'P/E ${detail.peBasis}' : 'P/E TTM',
            detail.peRatio,
            decimals: 1,
            suffix: 'x',
          ),
          if (detail.peRatio != null && detail.peSource.isNotEmpty)
            _textRow('Fonte P/E', detail.peSource.replaceAll('_', ' ')),
          _dataRow(
            'Prezzo / ricavi',
            detail.priceToSales,
            decimals: 1,
          ...
          if (detail.priceToSales != null && detail.priceToSalesSource.isNotEmpty)
            _textRow('Fonte prezzo / ricavi', detail.priceToSalesSource.replaceAll('_', ' ')),
          _dataRow(
          ...
          _textRow(
            'Capitalizzazione',
            formatCompactMoney(detail.marketCap, detail.reportingCurrency.isNotEmpty
                ? detail.reportingCurrency
                : detail.currency),
          ),
          if (detail.marketCap != null && detail.marketCapSource.isNotEmpty)
            _textRow('Fonte capitalizzazione', detail.marketCapSource.replaceAll('_', ' ')),
          ...
          if (detail.cashRunwayStatus == 'not_applicable_positive_fcf')
            _textRow('Autonomia di cassa', 'Non applicabile: free cash flow positivo'),
          const Divider(height: 24),
          _subsectionLabel('Qualità e provenienza dati'),
          _textRow(
            'Quotazione analizzata',
            '${detail.providerTicker} · ${detail.currency}'
            '${detail.isPrimaryListing ? ' · principale' : ' · secondaria'}',
          ),
          if (detail.reportingCurrency.isNotEmpty &&
              detail.reportingCurrency.toUpperCase() != detail.currency.toUpperCase())
            _textRow('Valuta fondamentali', detail.reportingCurrency),
          _textRow('Fonte prezzo', dataSourceLabel(detail.priceSource)),
          _textRow('Stato prezzo', priceStatusLabel(detail.priceStatus)),
          _textRow(
            'Osservato',
            formatObservedAt(detail.priceObservedAt),
          ),
          _textRow('Fonte fondamentali', dataSourceLabel(detail.fundamentalsSource)),
          if (detail.fundamentalsAgeDays != null)
            _textRow('Freschezza fondamentali', fundamentalsAgeLabel(detail.fundamentalsAgeDays)),
          ...
          if (detail.dataValidationWarnings.isNotEmpty) ...[
            const SizedBox(height: 8),
            ...detail.dataValidationWarnings.map(
              (warning) => Padding(

...
class AnalysisPreferences {
  static const _currentKey = 'analysis_preferences_v2';
  static const _presetsKey = 'analysis_presets_v2';
  ...
  const AnalysisPreferences({
    this.minOpportunity = 45,
    this.minAnomaly = 20,
    this.minConfidence = 25,
    this.maxValueTrap = 65,
    this.topN = 30,
    this.scanLimit = 200,
    this.catalystTopN = 10,
  ...
  factory AnalysisPreferences.fromJson(Map<String, dynamic> json) {
    return AnalysisPreferences(
      minOpportunity: (json['minOpportunity'] as num?)?.toDouble() ?? 45,
      minAnomaly: (json['minAnomaly'] as num?)?.toDouble() ?? 20,
      minConfidence: (json['minConfidence'] as num?)?.toDouble() ?? 25,
      maxValueTrap: (json['maxValueTrap'] as num?)?.toDouble() ?? 65,
      topN: (json['topN'] as num?)?.toInt() ?? 30,
      scanLimit: (json['scanLimit'] as num?)?.toInt() ?? 200,
      catalystTopN: (json['catalystTopN'] as num?)?.toInt() ?? 10,
